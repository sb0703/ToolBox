...
export const useUserStore = defineStore("user", {
  ...
  actions: {
    ...
    pinTool(id: string) {
      if (this.profile.pinnedToolIds.includes(id)) return;
    ...
    togglePin(id: string) {
      if (this.profile.pinnedToolIds.includes(id)) this.unpinTool(id);
      else this.pinTool(id);
    },
    addRecent(id: string) {
    ...
    renameFavoriteGroup(id: string, name: string) {
    ...
    deleteFavoriteGroup(id: string) {
      this.profile.favorites = this.profile.favorites.filter(
        (g) => g.id !== id
      );
      this.persist();
    },
    addToolToFavorite(groupId: string, toolId: string) {
    ...
    removeToolFromFavorite(groupId: string, toolId: string) {
      const group = this.profile.favorites.find((g) => g.id === groupId);
      if (!group) return;

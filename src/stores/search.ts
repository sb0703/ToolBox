import { defineStore } from 'pinia'
import { computed, ref } from 'vue'
import { useCatalogStore } from './catalog'
import { useUserStore } from './user'
import type { SearchQuery } from '@/utils/query'
export const useSearchStore = defineStore('search', () => {
  const catalog = useCatalogStore()
  const user = useUserStore()
  const query = ref('')
  const tag = ref<string | null>(null)
  const categoryId = ref<string | null>(null)
  const sort = ref<'az' | 'smart'>('smart')

  const results = computed(() => {
    const q = query.value.trim().toLowerCase()
    const list = catalog.tools.filter((t) => {
      const matchQ =
        !q || [t.name, t.summary, t.tags.join(' '), t.slug].some((v) => v.toLowerCase().includes(q))
      const matchTag = !tag.value || t.tags.includes(tag.value)
      const matchCat = !categoryId.value || t.categoryId === categoryId.value
      return matchQ && matchTag && matchCat
    })
    if (sort.value === 'az') return list.sort((a, b) => a.name.localeCompare(b.name))

    const pinned = user.pinnedSet
    const recents = user.profile.recentToolIds
    const score = (id: string) => {
      let s = 0
      if (pinned.has(id)) s += 100
      const recentIdx = recents.indexOf(id)
      if (recentIdx > -1) s += 50 - recentIdx
      return s
    }
    return [...list].sort((a, b) => {
      const sa = score(a.id)
      const sb = score(b.id)
      if (sa !== sb) return sb - sa
      return a.name.localeCompare(b.name)
    })
  })
  function setAll(q: SearchQuery) {
    if (q.q !== undefined) query.value = q.q || ''
    tag.value = q.tag ?? null
    categoryId.value = q.category ?? null
    sort.value = (q.sort as any) || 'smart'
  }
  return { query, tag, categoryId, sort, results, setAll }
})
export {}
